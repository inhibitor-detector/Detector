...
class HeartbeatService:
    ...
    def start_beating(self):
        ...
        self.detector.post_heartbeat(self.check_rfcat(), self.check_analyzer(), self.yard_is_running, self.check_memory(), True)
        self.require_healthy_system()
        while True:
            time.sleep(10)
            print("Heart beating...")            
            self.detector.post_heartbeat(self.check_rfcat(), self.check_analyzer(), self.yard_is_running, self.check_memory(), False)
            self.require_healthy_system()
